...
class KittiDataset(Dataset):
    ...
    def read_labels(self, filename, filename_manual):
        ...
        labels_plane[labels_plane != 0] += 2
        result_labels = np.clip(labels_road + labels_plane, 0, 1).astype(np.uint8)

        return torch.from_numpy(result_labels[None].T), labels_plane
    ...
    def __getitem__(self, idx):
        ...
        labels, labels_instance = self.read_labels(labels_path, manual_path)
        scene = scene[:self.scene_size]
        labels = labels[:self.scene_size]
        labels_instance = labels[:self.scene_size]
        
        return scene, labels, labels_instance
...
def get_kitti_filepaths(
    ...
    for drive in drives:
        ...
        folder_labels = [labels_original.format(drive) + '/' + scene.split("/")[-1].replace('.npy', '.label')[6:] for scene in folder_manual_labels]
